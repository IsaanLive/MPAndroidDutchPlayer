...
public class SearchActivity extends ListActivity {
    ...
    public void startsearch(){
		...
		toastmake("Zoeken naar: "+searchstring);
	...
	private class RetrieveSearch extends AsyncTask<Void, Void, Void>{
		...
		protected void onCancelled() {		
			...
			TextView failresults = (TextView)findViewById(R.id.failresults);
			failresults.setText("Zoeken is geannuleerd");
		...
		protected void onPreExecute() {
			...
			cancelDialog.setMessage("Zoeken naar Nummers...");
		...
		protected void onPostExecute(Void result) {
			if(ia == 0 && fail == 0){
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					failresults.setText("Het lijkt erop dat u niet over een actieve internetverbinding beschikt of dat u een zwak signaal heeft. Schakel over op uw wifi of op data en probeer het opnieuw.");
		        }else{
					failresults.setText("Geen Resultaten Gevonden");
		        }
			}else if(ia == 0 && fail > 0){
				fail = 0;
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					...
					failresults.setText("Het lijkt erop dat er een fout is opgetreden bij het zoeken. Probeer het opnieuw");
		        }
			}else{
				setListAdapter(adapter);
			}
		...
		public void getDilandau(String search) throws IOException, MalformedURLException, NullPointerException{
			...
			if(!sb.toString().contains("Geen nummers gevonden")){
