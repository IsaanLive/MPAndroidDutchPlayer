...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(totaldownloading >= 1){
        	text.setText("Totaal aantal downloads: "+totaldownloading);
        }else{
        	text.setText("Totaal aantal downloads: 0");
        }
        ...
        this.clearall.setOnClickListener(new OnClickListener() {
        	public void onClick(View v) {
        		...
        		toastmake("Voltooide & Mislukte Downloads opnieuw instellen");
        	}
        });
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("Download in de wachtrij");
		    		}else if(a == 0){
		    			progress.setText("Gedownload");
		    		}else if(a == -1){
		        		progress.setText("Download mislukt");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				builder.setMessage("Download Optie")
				       .setCancelable(false)
				       .setNegativeButton("Verwijderen", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("MP3 Download Verwijderd");
				           }
				       })
				       .setNeutralButton("Opnieuw proberen", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   mDbHelper.updateDownloaded(ida, "1"); 
				        	   refreshlist();
				        	   toastmake("Instellen om opnieuw te proberen");
				           }
				       })
				...
				final CharSequence[] items = {"MP3 Openen", "Instellen als Ringtone", "Opnieuw downloaden", "Verwijderen", getString(R.string.btn_cancel_name)};
	
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle("Kies een optie");
				builder.setItems(items, new DialogInterface.OnClickListener() {
				    public void onClick(DialogInterface dialog, int item) {
				    	if(items[item] == "MP3 Openen"){
				    	    try {
				    	        //launch intent
				    	    	...
				    	    	toastmake("Fout bij het Laden van de MP3");
				    	        e.printStackTrace();
				    	    }
				    	}else if(items[item] == "Instellen Als Ringtone"){
				    	...
				    	}else if(items[item] == "Verwijderen"){
							AlertDialog.Builder builder = new AlertDialog.Builder(DownloadingActivity.this);
							builder.setMessage("Weet u zeker dat u wilt verwijderen: " + title + " ?")
							       .setCancelable(false)
							       .setPositiveButton("Ja", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	    ...
							        	    mDbHelper.deleteDownload(ida);
							        	    refreshlist();
							                toastmake("MP3 Verwijderd");
							           }
							       })
							       .setNegativeButton("Nee", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	   toastmake("MP3 Niet Verwijderd");
							               dialog.cancel();
							           }
							       });
							AlertDialog alert = builder.create();
							alert.show();
				    	}else if(items[item] == "Opnieuw downloaden"){
				    		mDbHelper.updateDownloaded(ida, "1"); 
				    		refreshlist();
				    		toastmake("Instellen om Opnieuw te Proberen");
				    	}
				...
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setMessage("Downloadoptie")
				       .setCancelable(false)
				       .setNegativeButton("Verwijderen", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				...
				builder.setMessage("Downloadoptie")
				       .setCancelable(false)
				       .setNegativeButton("Download Stopzetten", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   if(sid == 1){
				                ...
				                toastmake("MP3 Download Gestopt");
				           }
				       })
				       .setPositiveButton(getString(R.string.btn_cancel_name), new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "Momenteel aan het Downloaden", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "In afwachting van Download", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "Voltooide Downloads", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "Mislukte Downloads", 100));
